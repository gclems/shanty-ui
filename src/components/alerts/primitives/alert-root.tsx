...
const alert = tv({
    base: cn(
        "relative grid w-full border rounded-sm px-6 py-2",
        "gap-y-0.5 items-start ",
        "has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current"
    ),
    variants: {
        color: {
...
const AlertPrimitiveRoot = ({ color, className, ...props }: ComponentProps<'div'> & {
    color?: 'info' | 'warning' | 'error';
}) => {
    return <div role="alert" className={alert({ color, class: className })} {...props} />
}
